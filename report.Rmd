...
The dataset is an online questionaire conducted in 2012, with 19719 participants. 50 different survey questions relating to personality using Likert scaling were asked and each respondant had to rank their response from 1, they disagree, to 5, they agree (0 meant the question was missed). The questions were not asked in any particular order. Other questions that were asked, were the race of the person, a categorical value, the age of the participant, a binary variable if the participant was fluent in English, the gender of the person, and finally their dominant writing hand. 

Each question is designed to reveal a different latent variable.

Some critisms of the validity of the data include the presence of acquience bias, where survey participants have a tendency to agree with a disproportiate number of questions, irrespective of the content of the question. This is managed by using questions with opposite meanings, for example, a person who answers agree to question "E5 I start conversations" is not going to also write agree to "E6 I have little to say".
...
Global Factor
When all the questions are loaded onto a global factor the
```{r echo=FALSE, results=FALSE, warning=FALSE}
# Global Factor Model
...
models$m3 <- 'E =~ E1 + E2 + E3 + E4 + E5 + E6 + E7 + E8 + E9 + E10
...
E ~ US+GB+FR+IT+AU+DE+CN+FI+IE+AT+CZ+GR+ID+NL+NO+ES+SE+RO+IN+DK+CA+BE+BR
N =~ N1 + N2 + N3 + N4 + N5 + N6 + N7 + N8 + N9 + N10
N ~ US+GB+FR+IT+AU+DE+CN+FI+IE+AT+CZ+GR+ID+NL+NO+ES+SE+RO+IN+DK+CA+BE+BR
A =~ A2 + A1 + A3 + A4 + A5 + A6 + A7 + A8 + A9 + A10
A ~ US+GB+FR+IT+AU+DE+CN+FI+IE+AT+CZ+GR+ID+NL+NO+ES+SE+RO+IN+DK+CA+BE+BR
C =~ C1 + C2 + C3 + C4 + C5 + C6 + C7 + C8 + C9 + C10
C ~ US+GB+FR+IT+AU+DE+CN+FI+IE+AT+CZ+GR+ID+NL+NO+ES+SE+RO+IN+DK+CA+BE+BR
O =~ O1 + O2 + O3 + O4 + O5 + O6 + O7 + O8 + O9 + O10
O ~ US+GB+FR+IT+AU+DE+CN+FI+IE+AT+CZ+GR+ID+NL+NO+ES+SE+RO+IN+DK+CA+BE+BR' 


# Five Factor Model (with correlated residuals)

models$m5 <- 'E =~ E1 + E2 + E3 + E4 + E5 + E6 + E7 + E8 + E9 + E10
N =~ N1 + N2 + N3 + N4 + N5 + N6 + N7 + N8 + N9 + N10
A =~ A2 + A1 + A3 + A4 + A5 + A6 + A7 + A8 + A9 + A10
C =~ C1 + C2 + C3 + C4 + C5 + C6 + C7 + C8 + C9 + C10
O =~ O1 + O2 + O3 + O4 + O5 + O6 + O7 + O8 + O9 + O10
...
modificationindices(object = fits$m3, sort. = TRUE)
